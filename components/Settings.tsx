      ...
      <div className="flex flex-col gap-10">
        ...
        <SettingCard>
          <SectionHeader 
            ...
            subtitle="Gerenciamento de Herói"
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            >
              ...
              <div className="text-left">
                 <span className={`block cinzel text-xs font-bold uppercase tracking-widest ${isDark ? 'text-red-400' : 'text-red-800'}`}>Deslogar</span>
                 <span className={`block parchment-text text-[10px] opacity-60 ${isDark ? 'text-red-300' : 'text-red-900'}`}>Encerrar Sessão</span>
              </div>
            </button>
            ...
            >
              ...
              <div className="text-left">
                 <span className={`block cinzel text-xs font-bold uppercase tracking-widest ${isDark ? 'text-[#d4af37]' : 'text-[#3e2723]'}`}>Trocar Herói</span>
                 <span className={`block parchment-text text-[10px] opacity-60 ${isDark ? 'text-[#e8d5b5]' : 'text-[#5d4037]'}`}>Mudar Registro</span>
              </div>
            </button>
          </div>
        </SettingCard>
        ...
        <SettingCard className={`relative overflow-hidden ${isDark ? 'bg-gradient-to-br from-[#1a1a1a] to-[#2d1b0d]' : 'bg-gradient-to-br from-[#fdf5e6] to-[#fffacd]'}`}>
          ...
          <div className="flex flex-col md:flex-row items-center justify-between gap-6 relative z-10">
            <div className="flex items-center gap-6">
              ...
              <div className="text-left">
                <p className={`cinzel font-bold text-xl leading-none mb-1 ${isDark ? 'text-[#e8d5b5]' : 'text-[#3e2723]'}`}>Andarilho</p>
                <p className={`parchment-text text-sm italic opacity-60 ${isDark ? 'text-[#e8d5b5]' : 'text-[#3e2723]'}`}>Assinante Gratuito</p>
              </div>
            </div>
            <button 
        ...
        <SettingCard>
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            >
              ...
              <span className={`cinzel text-[10px] font-bold uppercase tracking-widest ${theme === 'light' ? 'text-[#8b4513]' : 'text-[#e8d5b5]'}`}>Dia</span>
            </button>
            ...
            >
              ...
              <span className={`cinzel text-[10px] font-bold uppercase tracking-widest ${theme === 'dark' ? 'text-[#d4af37]' : 'text-[#3e2723]'}`}>Noite</span>
            </button>
          </div>
        </SettingCard>
        ...
        <SettingCard>
          <SectionHeader 
            title="Língua do Reino" 
            subtitle="Tradução de Conceitos"
          ...
          <div className="flex flex-col gap-3">
            {[
              ...
              <button 
                ...
                className={`flex items-center gap-4 p-4 rounded-xl border-2 transition-all ${currentLang === lang.id ? (isDark ? 'bg-white/5 border-[#d4af37]' : 'bg-black/5 border-[#8b4513]') : 'border-transparent opacity-40 hover:opacity-80'}`}
              >
                <span className="text-xl">{lang.flag}</span>
                <span className={`parchment-text font-bold text-sm flex-grow text-left ${isDark ? 'text-[#e8d5b5]' : 'text-[#3e2723]'}`}>{lang.name}</span>
                {currentLang === lang.id && <div className={`w-2 h-2 rounded-full ${isDark ? 'bg-[#d4af37]' : 'bg-[#8b4513]'}`}></div>}
              </button>
            ))}
          </div>
        </SettingCard>
        ...
        <SettingCard>
          <SectionHeader 
            title="Biblioteca de Manuscritos" 
            subtitle="Preservação de Dados"
          ...
          <div className="flex flex-col md:flex-row items-center gap-8">
            ...
            <div className="flex-grow text-left">
              <p className={`parchment-text text-base mb-6 leading-relaxed ${isDark ? 'text-[#e8d5b5]/80' : 'text-[#5d4037]'}`}>
                Transforme os feitos épicos de <b>{character.name}</b> em um pergaminho sagrado (.json) para levá-lo a outras mesas.
              </p>
              <button 
        ...
        <SettingCard className={`border-dashed ${isDark ? 'border-[#d4af37]/30' : 'border-[#8b4513]/30'}`}>
          <SectionHeader 
            title="Mensagens de Coruja" 
            subtitle="Feedback e Sugestões"
          />
          <div className="flex flex-col items-center text-center">
            ...
            <p className={`parchment-text text-sm mb-6 leading-relaxed italic ${isDark ? 'text-[#e8d5b5]/60' : 'text-[#5d4037]/70'}`}>
              "Sua voz ecoa nas masmorras dos criadores. Envie uma coruja com suas sugestões ou relate as maldições (bugs) encontradas."
            </p>
            <a 
        ...
        <div className={`p-8 rounded-[2.5rem] border-2 transition-all ${isDark ? 'bg-[#1a1a1a]/40 border-white/5' : 'bg-black/5 border-[#8b4513]/10'}`}>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 text-center md:text-left">
            <div className="flex flex-col items-center md:items-start gap-4">
              <h4 className={`cinzel text-xs font-bold uppercase tracking-widest ${isDark ? 'text-[#d4af37]' : 'text-[#3e2723]'}`}>Segurança Rúnica</h4>
              <p className={`parchment-text text-[11px] leading-relaxed ${isDark ? 'text-[#e8d5b5]/50' : 'text-[#5d4037]/60'}`}>Todos os seus dados residem apenas na sua alma digital (LocalStorage). Nenhuma crônica é enviada para servidores externos.</p>
            </div>
            <div className="flex flex-col items-center md:items-start gap-4">
              <h4 className={`cinzel text-xs font-bold uppercase tracking-widest ${isDark ? 'text-[#d4af37]' : 'text-[#3e2723]'}`}>Mundo Conectado</h4>
              <p className={`parchment-text text-[11px] leading-relaxed ${isDark ? 'text-[#e8d5b5]/50' : 'text-[#5d4037]/60'}`}>Acesse seus heróis mesmo sem conexão com o plano astral. O Pergaminho é projetado para durar eras em qualquer lugar.</p>
            </div>
            <div className="flex flex-col items-center md:items-center justify-center gap-2">
              ...
              <span className={`cinzel text-[9px] font-bold uppercase tracking-[0.4em] opacity-20 ${isDark ? 'text-[#d4af37]' : 'text-[#8b4513]'}`}>v1.5.0 • CRÔNICA DIGITAL</span>
            </div>
          </div>
        </div>
      </footer>
