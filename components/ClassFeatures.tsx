...
const ClassFeatures: React.FC<Props> = ({ character, updateCharacter, onSelectSubclass, theme = 'light' }) => {
  ...
  const ActionBadge = ({ type }: { type?: FeatureInfo['actionType'] }) => {
    if (!type) return null;
    const colors = {
      'Ativa': isDark ? 'bg-red-950/40 text-red-400 border-red-900/40' : 'bg-red-50 text-red-800 border-red-200',
      'Ação': isDark ? 'bg-red-950/40 text-red-400 border-red-900/40' : 'bg-red-50 text-red-800 border-red-200',
      'Ação Bônus': isDark ? 'bg-amber-950/40 text-amber-400 border-amber-900/40' : 'bg-amber-50 text-amber-800 border-amber-200',
    ...
    const displayType = colors[type as keyof typeof colors] ? type : 'Passiva';

    return (
      <span className={`px-2 py-0.5 rounded text-[8px] cinzel font-bold uppercase tracking-widest border ${colors[displayType as keyof typeof colors]}`}>
        {displayType}
      </span>
    );
  };
