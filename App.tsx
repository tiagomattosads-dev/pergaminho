...
const App: React.FC = () => {
  ...
  // Controle para fechar o overlay de morte temporariamente para ver a ficha
  ...
  return (
    <div className={`fixed inset-0 flex flex-col overflow-hidden selection:bg-orange-200 ${theme === 'dark' ? 'dark-mode' : ''}`}>
      ...
      {/* NOVO AVISO DE MORTE COM TIPOGRAFIA APRIMORADA (Fora do Container Cinza) */}
      {isDead && showDeathOverlay && (
      ...
      {/* Botão flutuante para reabrir o aviso caso tenha sido fechado para visualização */}
      {isDead && !showDeathOverlay && (
        <button 
        ...
        >
          ...
          <span className="cinzel text-[10px] font-bold uppercase tracking-widest">Abrir Veredito</span>
        </button>
      )}
    </div>
  );
