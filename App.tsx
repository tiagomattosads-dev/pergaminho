...
const App: React.FC = () => {
  ...
  return (
    <div className={`fixed inset-0 flex flex-col overflow-hidden selection:bg-orange-200 ${theme === 'dark' ? 'dark-mode' : ''}`}>
      ...
      {/* Container de Filtros de Morte */}
      <div className={`absolute inset-0 flex flex-col transition-all duration-1000 ${isNearlyDead ? 'animate-pulse-gray' : isDead ? 'is-fully-gray no-borders' : ''}`}>
            ...
            <div className="flex flex-col md:flex-row justify-between items-stretch py-2 md:py-4 lg:py-6 gap-3 md:gap-6">
              ...
              <div className="flex-none w-full md:w-auto flex items-center gap-3 lg:gap-6">
                  ...
                  <div className="flex items-center gap-2 md:gap-5 mb-1.5 md:mb-3 relative z-10">
                    ...
                    <div className="flex-grow flex flex-col min-w-0">
                      ...
                      <div className="h-1.5 md:h-3 w-full bg-black/60 rounded-full border border-[#8b4513]/40 overflow-hidden shadow-[inset_0_2px_10px_rgba(0,0,0,0.5)] p-[1.5px] relative">
                        <div className="h-full bg-gradient-to-r from-[#5d4037] via-[#d4af37] to-[#f4e4bc] transition-all duration-1000 ease-out relative rounded-full" style={{ width: `${levelData?.progressPercent}%` }}></div>
                      </div>
                    </div>
                  </div>
                </div>
                ...
                <button 
                  ...
                  className={`hidden md:flex flex-none p-3.5 rounded-2xl border-2 transition-all duration-300 shadow-xl active:scale-95 ${activeTab === Tab.Settings || activeTab === Tab.Subscription ? 'bg-[#d4af37] border-[#fffacd] text-[#1a0f00]' : 'bg-[#1a0f00] border-[#8b4513]/60 text-[#d4af37]'}`}
                >
        ...
        <nav className="xl:hidden flex-none z-[100] bg-[#2d1b0d] border-t-2 border-[#8b4513] shadow-[0_-5px_20px_rgba(0,0,0,0.7)] flex backdrop-blur-md">
          ...
          <MobileNavButton tab={Tab.Inventory} label="Itens" icon={<svg className="w-6 h-6" fill="currentColor" viewBox="0 0 24 24"><path d="M18 7H17V6C17 4.34 15.66 3 14 3H10C8.34 3 7 4.34 7 6V7H6C4.34 7 3 8.34 3 10V20C3 21.66 4.34 23 6 23H18C19.66 23 21 21.66 21 20V10C21 8.34 19.66 7 18 7ZM9 6C9 5.45 9.45 5 10 5H14C14.55 5 15 5.45 15 6V7H9V6ZM15 13H9V11H15V13Z" /></svg>} />
          <MobileNavButton tab={Tab.Magic} label="Magias" icon={<MagicIconMobile />} />
      ...
      {/* Overlay de Morte - Renderizado FORA do container grayscaled */}
      {isDead && (
        <div className="fixed inset-0 z-[1000] flex items-center justify-center bg-black/90 backdrop-blur-xl p-6 text-center animate-in fade-in duration-1000">
          <div className="flex flex-col items-center max-w-lg">
            ...
            <h3 className="fantasy-title text-5xl text-red-600 mb-4 drop-shadow-md uppercase tracking-wider">
              {character?.name}, morreu...
            </h3>
            <p className="fantasy-title text-2xl text-[#f4e4bc] mb-12 opacity-90 leading-relaxed italic">
